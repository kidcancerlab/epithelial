...
macs <- RenameIdents(macs,
    `0` = "Spp1 Mac",
    `1` = "Spp1 Mac", # Test change from Intermed mono
    `2` = "Pre-DC",
    `3` = "Cycling",
    `4` = "Spp1 Mac",
    `5` = "Monocyte",
...
macs$macs_assignment <- Idents(macs) %>%
    factor(levels = c("Monocyte", "Spp1 Mac", "Cycling", "Interstitial", "Pre-DC", "DC1", "DC2"))

mac_genes <- c(
    "Plac8", "Spn", "Lst1",  # Tumor infiltrating monocytes
    "Ms4a7", "Ly6c2",  # Intermediate monocytes
    "Gpnmb", "Trem2", "Spp1",  # Anti-inflammatory Spp1+ macs
    "Top2a", "Hist1h1b", "Birc5",  # Cycling
    "Lyve1", "Mrc1",  "Ccl8",  # Tissue resident/interstitial
...
macs_only <- subset(macs, subset = tissue != "Healthy Lung") %>%
    subset(subset = macs_assignment == "Spp1 Mac")

macs_only_degs <- FindMarkers(macs_only, ident.1 = "Lung Metastasis", group.by = "tissue") %>%
    arrange(-avg_log2FC)

deg_macs_3 <- FindAllMarkers(macs)
