...
```{r}
normal_lung <- merge(normal_lung[[1]],
    normal_lung[2:length(normal_lung)],
    add.cell.ids = names(normal_lung))

# Adjust names of some meta to match our lab standards
normal_lung$id <- normal_lung$Sample_Name
normal_lung$src <- normal_lung$Sample_Source
normal_lung$type <- normal_lung$Sample_Type
normal_lung$method <- "cell"

# Simplify meta before merge
normal_lung@meta.data <- normal_lung@meta.data %>%
...
met_lung <- merge(met_lung[[1]],
    met_lung[2:length(met_lung)],
    add.cell.ids = names(met_lung))

lungs <- merge(normal_lung, met_lung) %>%
    NormalizeData() %>%
...
DimPlot(lungs, group.by = "id", split.by = "type")
r_feature_plot(met_lung, "COL1A1", split.by = "type")
```
