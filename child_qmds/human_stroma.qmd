...
if(!file.exists("output/rdata/normal_lung.qs")) {
    ...
    normal_lung <- readRDS("GSE227136/GSE227136.rds")[normal_names]
    normal_lung <- lapply(seq_along(normal_lung), function(i) {
        DefaultAssay(normal_lung[[i]]) <- "RNA"
        normal_lung[[i]] <- DietSeurat(normal_lung[[i]], assays = "RNA") %>%
            NormalizeData()
    })
...
r_feature_plot(met_stroma, "COL1A1", split.by = "type")
```
