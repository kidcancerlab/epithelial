...
if(!file.exists("misc/lungs_manual_assignment.qs")) {
    ...
    lungs <- RenameIdents(lungs,
        ...
        `16` = "Mononuclear",  # Tumor carryover?
        `17` = "Mononuclear",  # Stem/proliferating?
        `18` = "Granulocyte",
        `19` = "B",
        `20` = "Mononuclear",  # Stem/proliferating?
...
if(!file.exists("output/rdata/human_epithelial.qs")) {
    epi <- subset(lungs,
        manual_assignments == c("Alveolar Epithelial", "Stem/Proliferating"))
    ...
    epi <- DietSeurat(epi, assays = "RNA") %>%
        ...
        harmony::RunHarmony(group.by.vars = c("src"), lambda = c(1)) %>%
    ...
    epi <- subset(epi, idents = c("2", "5", "7"), invert = TRUE)
...
DotPlot(epi, features = epi_genes, dot.scale = 8, cols = c("lightgoldenrod", "darkred")) +
...
macs <- DietSeurat(macs, assays = "RNA") %>%
    ...
    ScaleData() %>%
    RunPCA(npcs = 20, verbose = FALSE) %>%
    harmony::RunHarmony(group.by.vars = c("id", "method"), lambda = c(1, 1)) %>%
...
r_feature_plot(macs, "MARCO")
r_feature_plot(macs, "MRC1")

# Clean up one last time (remove alveolar macs)
macs <- subset(macs, idents = c("0"), invert = TRUE)
...
r_feature_plot(lungs, "TREM2", split.by = "type", max.cutoff = 3)
