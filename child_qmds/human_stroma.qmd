...
if(!file.exists("misc/lungs_manual_assignment.qs")) {
    ...
    lungs <- RenameIdents(lungs,
        ...
        `13` = "Differentiating",
        `14` = "Stem/proliferating",
        `15` = "Smooth muscle cell",
        `16` = "Myeloid",
        `17` = "Granulocyte",
        `18` = "Upper airway cell",
        `19` = "Fibroblast",
        `20` = "Fibroblast",
        `21` = "B cell",
        `22` = "Endothelial cell",
        `23` = "Distal airway cell",
        `24` = "Fibroblast",
        `25` = "B cell",
        `26` = "Myeloid",
        `27` = "Distal airway cell",
        `28` = "Endothelial cell",
        `29` = "LowQ")
...
if(!file.exists("output/rdata/human_epithelial.qs")) {
    epi <- subset(lungs,
        manual_assignments == c("Distal airway cell", "Stem/proliferating"))
    epi <- SplitObject(epi, split.by = "type")
    min_cells <- lapply(epi, function(x) {length(Cells(x))}) %>%
        unlist() %>%
        min()
    for (i in seq_along(epi)) {
        epi[[i]] <- subset(epi[[i]], downsample = min_cells)
    }
    epi <- merge(epi[[1]], epi[[2:length(epi)]])
