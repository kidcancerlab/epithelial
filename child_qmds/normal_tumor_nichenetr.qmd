    ...
    data.frame(
        matrix(
            rep(
                NA,
                length(gene_lists) *
                    length(unique(epi_myeloid_data$coarse_cell_type))
            ),
            ncol = length(gene_lists)
        )
    )
colnames(anova_pval_df) <- names(gene_lists)
rownames(anova_pval_df) <- unique(epi_myeloid_data$coarse_cell_type)

for (downstream_targets in names(gene_lists)) {
    for (cell_type in unique(epi_myeloid_data$coarse_cell_type)) {
        sub_data <- subset(epi_myeloid_data, coarse_cell_type == cell_type)
        anova_pval <-
            (
                aov(
                    lm(unlist(sub_data[[downstream_targets]]) ~ sub_data$treatment)
                ) %>%
                summary()
            )[[1]][1, 5]
        anova_pval_df[cell_type, downstream_targets] <- anova_pval
    }
}

# Bonferroni correction
temp <-
    p.adjust(as.matrix(anova_pval_df)) %>%
    matrix(ncol = length(gene_lists)) %>%
    -log10(.) %>%
    as.data.frame()
colnames(temp) <- colnames(anova_pval_df)
rownames(temp) <- rownames(anova_pval_df)

write_tsv(temp, "output/tables/nichenetr/anova_neg_log10_pvals.tsv")
...
plot_name <-
    ...
    geom_text(aes(label = round(padj, 2))) +
...
ggsave(
    "output/figures/nichenetr/anova_neg_log10_pvals.pdf",
    plot_name,
    width = 15,
    height = 10
)
qs::qsave(plot_name, "output/figures/nichenetr/anova_neg_log10_pvals.qs")
