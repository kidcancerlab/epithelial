...
get_median_aucell <- function(sobj,
                              sobj_column,
                              group_id,
                              group_vector) {
    sobj[[sobj_column]][group_vector == group_id, ] |>
    median()
}

get_max_diff <- function(sobj,
                         downstream_targets,
                         groups_vector) {
    max(
        abs(
...
for (downstream_targets in names(gene_lists)) {
    ...
    parallel::mclapply(
        ...
        output <- data.frame(max_diff = NA, pval = NA, median_diff = NA)
        output$max_diff <-
            get_max_diff(sub_data, downstream_targets, sub_data$treatment)
            ...
            lapply(
                ...
                function(x) {
                    get_max_diff(
                        sub_data,
                        downstream_targets,
                        sample(sub_data$treatment)
                    )
                })

        output$pval <-
            sum(perm_max_diffs > output$max_diff) / n_perms

        output$median_diff <- unlist(perm_max_diffs) %>% median()

        return(output)
    }) %>%
        bind_rows()
    anova_pval_df[downstream_targets] <- stuff$pval

    anova_max_diff_df[downstream_targets] <- stuff$max_diff
}
...
write_tsv(temp, "output/tables/nichenetr/nichenetr_pvals_adjusted.tsv")
write_tsv(anova_max_diff_df, "output/tables/nichenetr/nichenetr_max_diff.tsv")
...
anova_max_diff_df %>%
    ...
    pheatmap::pheatmap(
        ...
        filename = "output/figures/nichenetr/nichenetr_max_diff_heatmap.pdf"
    )
...
ggsave(
    "output/figures/nichenetr/nichenetr_pvals.pdf",
    plot_name,
    width = 10,
    height = 8
)
qs::qsave(plot_name, "output/figures/nichenetr/nichenetr_pvals.qs")
```
