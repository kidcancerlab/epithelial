...
set.seed(1337)
gene_lists$random_genes <- sample(rownames(epi_myeloid_data), 30)
...
anova_pval_df <-
    data.frame(
        cell_type = unique(epi_myeloid_data$coarse_cell_type)
    )

get_anova_f <- function(dataset, aucell_output) {
    f_val <-
        (aov(
            lm(aucell_output ~ dataset$treatment)
        ) %>%
            summary())[[1]][1, 4]
    return(f_val)
}

run_aucell <- function(dataset,
                       gene_list,
                       permute = FALSE,
                       random_seed = 1337) {
    aucell_out <-
        AUCell::AUCell_run(Seurat::GetAssayData(dataset), gene_list) %>%
        AUCell::getAUC()
...
for (downstream_targets in names(gene_lists)) {
    ...
    parallel::mclapply(
        ...
        anova_f <-
            run_aucell(
                sub_data,
                gene_lists[[downstream_targets]]
            ) %>%
            as.numeric() %>%
            get_anova_f(sub_data, .)
        ...
        random_gene_lists <-
            lapply(
                ...
                function(x) {
                    sample(
                        rownames(sub_data),
                        length(gene_lists[[downstream_targets]])
                    )
                }
            )
        names(random_gene_lists) <- paste0("random_genes_", 1:n_perms)

# redo permutations, but permute the treatment column, then do pairwise comparisons between control and each treatment, calculating the difference in median
        ...
        perm_f_vals <-
            lapply(
                1:n_perms,
                function(x) {
                    get_anova_f(sub_data, aucell_perms[, x])
                }
            ) %>%
            as.numeric() %>%
            na.omit()

        return(sum(perm_f_vals > anova_f) / length(perm_f_vals))
    }) %>%
        unlist()
    anova_pval_df[downstream_targets] <- stuff
}

# Bonferroni correction
temp <-
    column_to_rownames(anova_pval_df, "cell_type") %>%
    as.matrix() %>%
    p.adjust() %>%
    matrix(ncol = length(gene_lists)) %>%
    #-log10(. + 0.001) %>%
    as.data.frame()
colnames(temp) <- names(gene_lists)
rownames(temp) <- anova_pval_df$cell_type

write_tsv(temp, "output/tables/nichenetr/anova_pvals.tsv")

plot_name <-
    temp %>%
    rownames_to_column("cell_type") %>%
    pivot_longer(-cell_type, names_to = "pathway", values_to = "padj") %>%
    mutate(
        signif = ifelse(
            padj > 0.05,
            "Not significant",
    ...
    labs(
        title = "AUCell pathway enrichment ANOVA p-values",
        x = "",
        y = ""
    ) +
    theme(plot.title = element_text(hjust = 0.5))
ggsave(
    "output/figures/nichenetr/anova_pvals.pdf",
    plot_name,
    width = 10,
    height = 8
)
qs::qsave(plot_name, "output/figures/nichenetr/anova_pvals.qs")
```
