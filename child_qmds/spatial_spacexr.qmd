...
coords <-
    lapply(
        Images(spatial_data),
        function(x) {
            GetTissueCoordinates(spatial_data, image = x) %>%
                select(-cell)
        }) %>%
    bind_rows()
...
```{r run_decomposition}
#| cache.vars: rctd_out
# Need to have the doSNOW package installed
rctd_obj <-
    spacexr::create.RCTD(my_data, spacexr_reference, max_cores = 10)
...
for (cell_type in all_ref$cell_type %>% unique()) {
    ...
    ggsave(
        paste0(
            "output/figures/spatial/spacexr_",
            cell_type,
            ".pdf"
        ),
        plot_name,
