...
ggsave("output/figures/mac_dim.pdf",
       width = 20,
       height = 8)
...
ggsave("output/figures/mac_cluster_counts.pdf",
       width = 8,
       height = 8)
...
tumor_only <-
    subset(immune_recurl,
           obj_name %in% c("K7M2", "F420", "tibia_F420"))
...
# test <-
#     lapply(kegg_ref,
#            function(x) sum(inflam_genes %in% x))
...
gsea_out %>%
    #filter(pathway %in% top_paths) %>%
    ggplot(aes(x = cluster, y = pathway, fill = NES)) +
    geom_tile() +
...
```{r eval=FALSE}
DimPlot(stuff, cols = c("red", "#FFFFFF"))
```