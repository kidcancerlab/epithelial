...
- Our data
    - stuff here
- GSE129605
    - bleo and nintendinib treated mice - 10X
    - Defining the Activated Fibroblast Population in Lung Fibrosis Using Single Cell Sequencing
    - PMID: 30848683
    - 10x
    - Has three gz files
- GSE132771
    - bleo treated mice - 10x
    - Single cell RNA-sequencing of normal and fibrotic lungs in mouse and human
    - PMID: 32317643
    - 10x
    - Has three gz files
- GSE145031
...
#GSE82158 - bleo treated sorted cells - RNAseq
#- GSE240134 - bleo treated mice - 10X
#    - BD rhapsody
...
# Load GEO raw data
scheduled cores 9, 6, 10, 7, 8, 12, 11, 14, 16, 18, 15, 19, 13, 17, 20, 2, 3, 1 encountered errors in user code, all values of the jobs will be affected
```{r load_raw, cache.vars = '', eval=TRUE}
sample_list <- read_tsv("misc/bleomycin_cutoffs.txt", show_col_types = FALSE)
...
sobj_list <-
    ...
    if (sample_list$input_format[i] == "three files") {
        sobj <-
            tenx_load_qc(paste0("input/",
                                sample_list$geo[i], "/",
                         ...
                         violin_plot = FALSE,
                         mt_pattern = "^M") %>%
            # Some of these samples don't have mt reads in them, which causes an
            # error, so I'm using a fake mt pattern to avoid that and then
            # recalculating the percent.mt column
            PercentageFeatureSet(pattern = gsub("_", "-", "^Mt-"),
                                 col.name = "percent.mt")
    } else if (sample_list$input_format[i] == "h5 file") {
        sobj <-
            tenx_load_qc(h5_file = paste0("input/",
                                          sample_list$geo[i], "/",
                                          sample_list$sample_name[i],
                                          ".h5"),
                         min_cells = 3,
            ...
            PercentageFeatureSet(pattern = gsub("_", "-", "^Mt-"),
                                 col.name = "percent.mt")
    } else if (sample_list$input_format[i] == "counts matrix") {
        counts <-
            read_csv(paste0("input/",
                            sample_list$geo[i], "/",
                            sample_list$sample_name[i],
                            ".csv.gz"),
    ...
    ggsave(paste0("output/figures/feature_hists/feature_hist_",
                  obj_name,
                  ".png"),
           width = 10,
           height = 10,
    ...
    qs::qsave(sobj,
              file = paste0("output/rdata/", obj_name, ".qs"))
    return(sobj)
})
...
qs::qsave(sobj_list,
          file = "output/rdata/sobj_list_bleo.qs")
...
qs::qsave(sobj_list,
          file = "output/rdata/sobj_list_ours.qs")
...
```{r eval=FALSE}
our_sobj_list <- qs::qread("output/rdata/sobj_list_ours.qs")
bleo_sobj_list <- qs::qread("output/rdata/sobj_list_bleo.qs")

all_things <-
    merge(our_sobj_list[[1]],
          c(our_sobj_list[2:length(our_sobj_list)],
            bleo_sobj_list)) %>%
    process_seurat()

all_things$group[is.na(all_things$group)] <- "our data"
all_things$geo[is.na(all_things$geo)] <- "our data"

qs::qsave(all_things,
          file = "output/rdata/all_things_sobj.qs")

DimPlot(subset(all_things,
               subset = grepl("^AT", cell_type)),
        group.by = "group")

test2 <-
    all_things[, grepl("Ciliated", all_things$cell_type)] %>%
    process_seurat()
...
test3 <-
    AverageExpression(test2, group.by = "group") %>%
    as.data.frame()

GGally::ggpairs(test3)

test <-
    Embeddings(subset(all_things,
                  cell_type == "Int Mf"),
               reduction = "pca")[, 1:9] %>%
    as.data.frame() %>%
    ...
    pivot_longer(-cell_id,
                 names_to = "pca",
                 values_to = "value") %>%
    left_join(all_things@meta.data %>%
                as.data.frame() %>%
                rownames_to_column("cell_id"),
                by = "cell_id")
...
test_harm <-
    harmony::RunHarmony(test2,
                        group.by.vars = "geo")

DimPlot(test_harm,
        group.by = c("group", "obj_name"),
        label = TRUE) +
    NoLegend()

test_limma <-
    limma::removeBatchEffect(GetAssayData(test2),
                             batch = test2$geo)
test_limma_sobj <-
    SetAssayData(test2, slot = "data", new.data = test_limma) %>%
    ScaleData() %>%
    RunPCA() %>%
    RunUMAP(dims = 1:30)

DimPlot(test_limma_sobj, group.by = c("geo", "group"))
DimPlot(test_limma_sobj,
        group.by = c("group", "obj_name"),
        label = TRUE) +
    NoLegend()
# this made crap output
...
hey also i was wondering if you could help me with some code to get a seurat object started from the "whole lung data sets" in from this paper: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE141259 
